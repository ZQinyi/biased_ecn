...
namespace hysup
{
    ...
    /* Concept: "Container with different functionality based on type" */
    template <typename T>
    ...
    public:
        ...
        void update_policy_state(int32_t this_addr);
    ...
    struct Stats
    {
        ...
        /* Variables */
        static uint64_t num_ticks_;
        static uint64_t num_data_pkts_sent_;                  // senders
        static uint64_t num_data_pkts_of_SRPT_msg_sent_[3];   // senders. Num pkts of the smallest msg, the second smallest, and the third smallest (in terms of bytes remaining)
        static double ratio_data_pkts_of_SRPT_msg_sent_[3];   // senders
        static uint64_t num_credit_pkts_sent_;                // receivers
        static uint64_t num_credit_pkts_of_SRPT_msg_sent_[3]; // receivers. Num pkts of the smallest msg, the second smallest, and the third smallest (in terms of bytes remaining)
        static double ratio_credit_pkts_of_SRPT_msg_sent_[3]; // receivers
        static int total_budget_bytes_;                       // receivers
        static uint32_t total_credit_backlog_;                // senders
        static uint32_t total_bif_rec_;                       // receivers
        static double total_avg_host_marked_ratio_;           // receivers
        static uint64_t uplink_busy_count_;                   // number of polls when there were data in the data_backlog
        static uint64_t uplink_idle_while_outbound_count_;    // number of polls when there were no data in the data_backlog and outbound messages exist
        static double total_ratio_uplink_busy_;
        static double total_ratio_uplink_idle_while_data_;
    };
}
#endif
