...
void R2p2CCHybrid::init()
{
    ...
    bind("srpt_ratio_", &srpt_ratio_);
    bind("receiver_policy_", &receiver_policy_);     // policy used by receiver to multiplex grants
    bind("account_unsched_", &account_unsched_);     // policy used by receiver to multiplex grants
    ...
    bind("sender_srpt_", &sender_srpt_);
    ...
    if (sender_srpt_ == 1)
        receivers_->set_policy_weights((1 - srpt_ratio_), srpt_ratio_);
    else
    ...
    last_poll_trace_ = Scheduler::instance().clock();
    ...
    slog::log2(debug_, this_addr_, "R2p2CCHybrid::init():", debug_, sender_srpt_, receiver_policy_, account_unsched_, unsolicited_thresh_bytes_,
               ...
               sender_algo_, eta_, wai_, max_stage_, unsolicited_limit_senders_, unsolicited_burst_when_idle_, priority_flow_, srpt_ratio_);
...
void R2p2CCHybrid::send_data()
{
    ...
    // RMEMBER: PROBLEM when sender self-allocates credit but doesn't tell receiver.

    receivers_->update_policy_state(this_addr_); // called for every packet
    ...
    //     size_t order = outbound_->msg_order_size(msg_state);
    //     if (order < 3)
    //     {
    ...
    {
        slog::log4(debug_, this_addr_, "Sending packet of msg",
...
void R2p2CCHybrid::received_data(Packet *pkt, hysup::InboundMsgState *msg_state)
{
    ...
    {
        slog::log4(debug_, this_addr_, "Removing inbound message state");
...
hysup::SenderState *R2p2CCHybrid::update_sender_state(Packet *pkt)
{
    ...
    // std::cout << "Opportunity to reset" << std::endl;
...
void R2p2CCHybrid::update_sn_state(Packet *pkt, hysup::SenderState *state)
{
    if (sender_algo_ == 0 || sender_algo_ == 1 || sender_algo_ == 2 || sender_algo_ == 3) // DCTCP (2 is for 3d bit)
    {
        ...
        else if (sender_algo_ == 3) // reset when SRPT
        {
            ...
            if (r2p2_hdr->qlen() == 0)
            {
                slog::log4(debug_, this_addr_, "Resetting host_max_srpb_bytes_ to", static_cast<int>(srpt_ratio_ * max_srpb_));
                state->host_max_srpb_bytes_ = static_cast<int>(srpt_ratio_ * max_srpb_);
            }
        }
    }
...
void R2p2CCHybrid::poll_trace()
{
    if (do_trace_)
    {
        if (num_polls_ % POLL_SAMPLE_GAP == 0)
            trace_state("pol", -1);
    }
}
