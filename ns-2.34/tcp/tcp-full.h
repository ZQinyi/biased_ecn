...
struct EcnStats {
    ...
    std::queue<std::tuple<double, int, int>> packetsInWindow;
    double alpha = 0.0;
    double beta = 0.0;
...
struct EcnStats {
    int totalPackets;
    int ecnMarkedPackets;
	double timeStamp;
	int senderCount;
	double alpha;
	std::queue<std::tuple<double, int, int>> packetsInWindow;
	// std::unordered_map<int, double> senderAddrs;

    EcnStats() : totalPackets(0), alpha(0), senderCount(0), ecnMarkedPackets(0), timeStamp(0) {}
};
*/
...
struct EcnStats {
    ...
    double x_est = 0.0; // The estimated state value
    double P = 1.0; // The estimated error covariance
    double Q = 0.1; // Process noise covariance
    double R = 0.1; // Measure noise covariance
};
*/
